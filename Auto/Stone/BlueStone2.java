...
public void runOpMode() throws InterruptedException {
  ...
  while(opModeIsActive()) {
    ...
    // Second Stone
    int valTune = 50; //deltaPos???
    moveBackward(valTune, LINEAR_SPEED_MAX, 1);
    sleep(DEFAULT_METHOD_DELAY);

    strafeRight(14, SIDEWAYS_SPEED_NORM, 1);
    sleep(DEFAULT_METHOD_DELAY);

    clampBlock();
    sleep(DEFAULT_SERVO_DELAY);

    strafeLeft(5, SIDEWAYS_SPEED_MAX, 1);
    sleep(DEFAULT_METHOD_DELAY);
    ...
    valTune = 10; //deltaPos??
    moveBackward(10 + deltaPos, LINEAR_SPEED_MAX, 1);
    sleep(DEFAULT_SERVO_DELAY);
