...
public class AutoSystem extends LinearOpMode {
  ...
  Servo leftClamp, rightClamp, blockServo;
  ...
  private static final double INCHES_PER_ROT = (COUNTS_PER_REV) / (DIAMETER * Math.PI);
  ...
  private static final double RAMP_INCREMENT = 0.01;
  ...
  public void moveForward(double inches, double speed, double coeff) {
    ...
    motors[0].setTargetPosition(-target); motors[1].setTargetPosition(-target);
    motors[2].setTargetPosition(target); motors[3].setTargetPosition(target);
    runMotors(speed);

    // Wait while moving
    while(opModeIsActive() && motors[0].isBusy() && motors[1].isBusy() && motors[2].isBusy() && motors[3].isBusy()) {

      double correction = checkDirection(0.06) * speed;
  ...
  public void strafeLeft(double inches, double speed, double coeff) {
    ...
    // Disable back brakes to counteract drifting while strafing
    ...
    while(opModeIsActive() && motors[0].isBusy() && motors[1].isBusy() && motors[2].isBusy() && motors[3].isBusy()) {
      ...
      double correction = checkDirection(0.02);
  ...
  public void clampBlock() {
    servos[2].setPosition(0.13);
  }
  /* Releases Block */
  public void releaseBlock() {
    servos[2].setPosition(0.54);
  }
